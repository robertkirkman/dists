...
repo_ready() {
    local full_repo="$1"
    local repo_readme_url="https://raw.githubusercontent.com/$full_repo/refs/heads/main/README.md"
    local template_readme_url="https://raw.githubusercontent.com/$dists_template_repo/refs/heads/main/README.md"

    if [[ "$(curl -s "$repo_readme_url")" == "$(curl -s "$template_readme_url")" ]]; then
        return 0
    else
        return 1
    fi
